...
export default async function ProfilePage() {
  ...
  return (
    <div className="min-h-screen font-sans" style={{ background: '#0A192F' }}>
      {/* Bottom tab bar — mobile only, mirrors dashboard pattern */}
      ...
      <div className="mx-auto max-w-md md:max-w-2xl lg:max-w-5xl px-4 pb-28 md:pb-10">
        ...
        <header className="flex items-center justify-between py-5">
          ...
          {/* Top nav — tablet + desktop (same as dashboard) */}
          <DashboardNav variant="top" />
          {/* Back link — mobile only (top nav replaces it on md+) */}
          <Link
            href="/dashboard"
            className="md:hidden text-xs font-medium px-3 py-1.5 rounded-full transition-colors"
            style={{
        ...
        <div className="md:grid md:grid-cols-[280px_1fr] md:gap-8 md:items-start">
          ...
          {/* ── User card ──────────────────────────────── */}
          <div
            className="rounded-2xl p-5 mb-6 md:mb-0 flex items-center gap-4 md:sticky md:top-8"
            style={{
